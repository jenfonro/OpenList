				...
				for retry := 0; retry < 3; retry++ {
					...
					case <-ticker.C:
						if ossToken, err = d.client.GetOSSToken(); err != nil { // 到时重新获取ossToken
							errCh <- errors.Wrap(err, "刷新token时出现错�?)
						}
					default:
					}
...
LOOP:
	...
	// 不知道啥原因，oss那边分片上传不计算sha1，导�?15服务器校验错�?
...
func SplitFile(fileSize int64) (chunks []oss.FileChunk, err error) {
	for i := int64(1); i < 10; i++ {
		if fileSize < i*utils.GB { // 文件大小小于iGB时分为i*1000�?
	...
	if fileSize > 9*utils.GB { // 文件大小大于9GB时分�?0000�?
