...
func (d *PikPak) login() error {
	...
	// 使用 用户填写�?CaptchaToken —————�?(验证后的captcha_token)
...
// RefreshCaptchaTokenAtLogin 刷新验证码token(登录�?
...
// RefreshCaptchaTokenInLogin 刷新验证码token(登录�?
	...
	for i := 0; i < ThreadsNum; i++ {
		go func(threadId int) {
			...
			for chunk := range chunksCh {
				var part oss.UploadPart // 出现错误就继续尝试，共尝�?�?
...
LOOP:
	...
	if _, err = bucket.CompleteMultipartUpload(imur, parts, OssOption(params)...); err != nil && !errors.Is(err, io.EOF) {
		// 当文件名含有 &< 这两个字符之一时响应的xml解析会出现错误，实际上上传是成功�?
...
func SplitFile(fileSize int64) (chunks []oss.FileChunk, err error) {
	for i := int64(1); i < 10; i++ {
		if fileSize < i*utils.GB { // 文件大小小于iGB时分为i*100�?
	...
	if fileSize > 9*utils.GB { // 文件大小大于9GB时分�?000�?
