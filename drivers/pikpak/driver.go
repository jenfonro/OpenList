...
func (d *PikPak) Init(ctx context.Context) (err error) {
	...
	if d.Addition.RefreshToken != "" {
		...
		// 如果没有填写RefreshToken，尝试登�?获取 refreshToken
...
func (d *PikPak) Put(ctx context.Context, dstDir model.Obj, stream model.FileStreamer, up driver.UpdateProgress) error {
	...
	// web 端上�?返回的endpoint �?`mypikpak.net` | android 端上�?返回的endpoint �?`vip-lixian-07.mypikpak.net`·
	if d.Addition.Platform == "android" {
		params.Endpoint = "mypikpak.net"
	}

	if stream.GetSize() <= 10*utils.MB { // 文件大小 小于10MB，改用普通模式上�?
